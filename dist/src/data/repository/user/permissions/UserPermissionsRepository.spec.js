...
const UserPermissionsRepository_1 = require("./UserPermissionsRepository");
...
const makeSut = () => {
    ...
    const sut = new UserPermissionsRepository_1.PrismaPermissionsUserRepository(prismaMock);
    return {
        sut,
        prismaMock
    };
...
describe('PrismaPermissionsUserRepository', () => {
    ...
    it('should throw error when inserting existing permissions to user', async () => {
        ...
        await expect(sut.insertPermissionToUser(permissionsUser)).rejects.toThrowError('[ENTITY- PERMISSIONS_USER]: Relação de permissões de usuário já existe');
    });
    it('should update permissions to user', async () => {
        const { sut, prismaMock } = makeSut();
        const permissionsUserId = 1;
        const updatedPermissionsUser = {
    ...
    it('should throw error when updating non-existing permissions to user', async () => {
        ...
        await expect(sut.updatePermissionToUser(permissionsUserId, updatedPermissionsUser)).rejects.toThrowError('[ENTITY- PERMISSIONS_USER]: Relação de permissões de usuário não encontrada');
    });
    ...
    it('should throw error when deleting non-existing permissions to user', async () => {
        ...
        // @ts-expect-error
        prismaMock.permissionsUser.findUnique.mockResolvedValue(null);
        await expect(sut.deletePermissionToUser(permissionsUserId)).rejects.toThrowError('[ENTITY- PERMISSIONS_USER]: Relação de permissões de usuário não encontrada');
    });
});
